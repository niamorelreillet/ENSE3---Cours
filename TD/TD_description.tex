...
Pour faciliter la réalisation de ce TD (en particulier, les questions des \Sects{rad}{param} qui ne peuvent pas être traitées analytiquement), il est accompagné de notebooks Jupyter (voir \href{https://mybinder.org/v2/gh/niamorelreillet/ENSE3-Cours/main?urlpath=lab}{https://mybinder.org/v2/gh/niamorelreillet/ENSE3-Cours/main?urlpath=lab}).
\end{abstract}
...
Plot the lateral flux profile $\varphi_{lat}^{ox}(z) = \phi_{lat}^{ox} f(z)$ along the vessel wall (considering the following shape function $f(z)$) and compare it for $z=H^{ox}$ to the pessimistic value of 1.5MW.m$^{-2}$ for the critical heaf flux associated with the external cooling of the vessel wall
...
Pour une cote $z \in [0,H^{ox}]$ donnée, on considèrera que le flux ``local'' $\varphi_{lat}^{ox}(z)$ est dissipé par conduction unidimensionnelle ``plan'' dans l'épaisseur $e_c(z)$ de la croûte réfractaire et l'épaisseur $e_v(z)$ de la paroi de la cuve (\textit{cf.} \Fig{cv}). 
...
\Q{En écrivant l'expression du flux conductif stationnaire dans l'épaisseur de la cuve, calculez la température $T_{c/v}$ d'interface entre la croûte et la cuve}
...
\Q{En écrivant l'expression du flux conductif stationnaire dans l'épaisseur de la croûte, calculez et tracez $e_c(z)$}
